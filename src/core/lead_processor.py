    ...
    async def process_lead_message(self, lead_phone: str, message: str) -> str:
        ...
        try:
            ...
            if extracted_info:
                ...
                updated_lead = await self.lead_repository.update(lead_phone, extracted_info)
                if updated_lead:
                    lead = updated_lead
                    print(f"[DEBUG] Lead updated successfully. Current lead data:")
                    for field in [
                        "move_in_date", "price", "beds", "baths", "location",
                        "amenities", "tour_availability", "tour_ready"
                    ]:
            ...
            await self.lead_repository.add_message_to_history(lead_phone, message, "lead")
            ...
            # Check if tour availability was just provided - trigger manager response
                ...
                extracted_info and extracted_info.get("tour_availability") and
                not lead.tour_availability
            ):
                ...
                if self.agent_phone:
                    ...
                    agent_sms_success = await self.messaging_service.send_agent_notification(
                        self.agent_phone, agent_message
                    )
                    if agent_sms_success:
                        print(f"Agent notification sent to {self.agent_phone}: {agent_message}")
                    else:
                        print(f"Failed to send agent notification to {self.agent_phone}")
                else:
                    print("No AGENT_PHONE_NUMBER configured - skipping agent notification")
                
                ai_response = ("Perfect! I have all the information I need. "
                ...
                missing_optional = await self.lead_repository.get_missing_optional_fields(lead)
                needs_tour_availability = await self.lead_repository.needs_tour_availability(lead)
                ...
                if (
                    not lead.next_follow_up_time and
                    not lead.follow_up_paused_until and
                    missing_fields
                ):
            ...
            if success:
                ...
                await self.lead_repository.add_message_to_history(lead_phone, ai_response, "ai")
